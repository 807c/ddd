    ...
    try:
        ...
        if local and local.combat_avatar:
            try:
                ...
                # ── Skin enforce — single source of truth: _DH_GUN_SAVED ──────
                # Runs throttled (every 90 ticks ~1.5s): if an equipped gun has a saved
                # skin in _DH_GUN_SAVED and the current skin doesn't match, re-apply.
                # Throttled to avoid hammering the engine every tick during weapon swaps
                # which causes AV crashes on stale weapon case pointers.
                if not hasattr(_b, '_DH_ENFORCE_COUNTER'): _b._DH_ENFORCE_COUNTER = 0
                _b._DH_ENFORCE_COUNTER += 1
                if hasattr(_b, '_DH_GUN_SAVED') and _b._DH_GUN_SAVED and _b._DH_ENFORCE_COUNTER % 90 == 0:
                    # Re-fetch weapon cases fresh this tick — never reuse refs from above
                ...
                # Melee enforce — uses weapon_id as key, not gun_id
                # Only runs on the same throttle cycle as gun enforce (~1.5s)
                # to avoid AV on stale melee weapon case pointers during skin swap.
                try:
                    saved = getattr(_b, '_DH_SKIN_SAVED_MELEE', None)
                    if saved and _wc_melee and _b._DH_ENFORCE_COUNTER % 90 == 0:
        ...
        if getattr(_b, '_DH_SKIN_BRIDGE_DIRTY', True) and local and local.combat_avatar:
            try:
                ...
                # All known gun_ids in BloodStrike
        ...
        if getattr(_b, '_DH_SKIN_NEXT_MAIN', False):
            ...
            if lst:
                ...
                _apply_skin(local.combat_avatar.GetMainGunWeaponCase(), lst[_b._DH_SKIN_IDX_MAIN], 'main')
        ...
        if getattr(_b, '_DH_SKIN_NEXT_SUB', False):
            ...
            if lst:
                ...
                _apply_skin(local.combat_avatar.GetSubGunWeaponCase(), lst[_b._DH_SKIN_IDX_SUB], 'sub')
        ...
        if getattr(_b, '_DH_SKIN_NEXT_MELEE', False):
            ...
            if lst:
                ...
                try:
                    ...
                    if wc: _apply_skin(wc, lst[_b._DH_SKIN_IDX_MELEE], 'melee')
                except: pass
