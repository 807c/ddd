...
def esp_tick(*args):
    ...
    try:
        ...
        if getattr(_b, '_DH_SKIN_LIST_DIRTY', True) and local and local.combat_avatar:
            # ── Async skin list build ─────────────────────────────────────────
            # GetAllGunSkinItemList is slow — running it in esp_tick blocks the
            # game's Python thread for multiple frames → micro-freeze + ESP miss.
            # We snapshot the combat_avatar ref and gun_ids NOW (safe, cheap),
            # then do the heavy work in a daemon thread. The results are written
            # back to builtins atomically when done — esp_tick never blocks.
            _b._DH_SKIN_LIST_DIRTY = False  # clear immediately so we don't re-trigger
            ...
            try:
                ...
                # Snapshot everything we need from the game thread NOW
                _ca_snap   = local.combat_avatar
                ...
                def _build_lists_bg():
                    try:
                        ...
                        def _build_melee():
                            try:
                                wc = _ca_snap.GetMeleeWeaponCase()
                                if not wc: return []
                        ...
                        lst_main  = _build('GetMainGunWeaponCase')
                        lst_sub   = _build('GetSubGunWeaponCase')
                        lst_melee = _build_melee()

                        # Only write back if gun_ids haven't changed again while we ran
                        ...
                        # else gun changed again — result is stale, discard silently
                    except: pass

                t = _threading.Thread(target=_build_lists_bg, daemon=True)
                t.start()
            except: pass
        ...
        # ── Build ALL-GUNS skin bridge file (background thread) ──────────────
        if getattr(_b, '_DH_SKIN_BRIDGE_DIRTY', True) and local and local.combat_avatar \
            ...
            try:
                ...
                _threading2.Thread(target=_write_bridge_bg, daemon=True).start()
            except: pass
