...
if not hasattr(_b, '_DH_SKIN_ENABLED'): _b._DH_SKIN_ENABLED = False  # disabled by default
...
# ── Warmup tick counter (pour bridge scan uniquement) ────────────────────────
# Le bridge scan des 44 guns est différé de 300 ticks (~5s) pour laisser
# le jeu finir d'initialiser ses weapon cases avant les 88+ appels C++.
if not hasattr(_b, '_DH_ENFORCE_COUNTER'):
    _b._DH_ENFORCE_COUNTER = -300
else:
    _b._DH_ENFORCE_COUNTER = -300
...
def get_bone(cam, ent, bone):
    try:
        ...
        sp = cam.GetScreenPointFromWorldPoint(m.GetBoneWorldPosition(bone))
        if sp and sp.z != -1.0:
...
def is_visible(space, ecam, ent, bone='biped Head'):
    try:
        ...
        head   = m.GetBoneWorldPosition(bone)
...
def esp_tick(*args):
    ...
    try:
        # Tick counter — used only for bridge scan delay, nothing else
        _b._DH_ENFORCE_COUNTER = getattr(_b, '_DH_ENFORCE_COUNTER', -300) + 1
        ...
        _gun_changed = False
        # Track gun_id changes to know when to rebuild the skin list.
        if local and local.combat_avatar:
            try:
                ...
                if _gm != _b._DH_SKIN_GUN_MAIN or _gs != _b._DH_SKIN_GUN_SUB:
                    ...
                    # Don't set LIST_DIRTY here — defer 2 ticks so the new
                    # weapon case has time to fully init C++-side before we
                    # call GetAllGunSkinItemList on it.
                    _b._DH_SKIN_LIST_DIRTY = False
                    _b._DH_SKIN_DIRTY_DELAY = 2
        ...
        # Countdown the deferred dirty flag
        if getattr(_b, '_DH_SKIN_DIRTY_DELAY', 0) > 0:
            _b._DH_SKIN_DIRTY_DELAY -= 1
            if _b._DH_SKIN_DIRTY_DELAY == 0:
                _b._DH_SKIN_LIST_DIRTY = True

        if getattr(_b, '_DH_SKIN_LIST_DIRTY', True) and local and local.combat_avatar:
            # ── Skin list build — synchronous in Cocos thread ─────────────────
            # IMPORTANT: tous les accès aux objets C++ du jeu (weapon cases,
            # item_proto, etc.) DOIVENT se faire ici dans esp_tick (thread Cocos).
            # Lancer un thread daemon qui accède à ces objets C++ → crash natif
            # non catchable : les objets peuvent être libérés à tout moment par
            # le GC ou un changement de scène, et Python ne protège pas contre ça.
            # GetAllGunSkinItemList est rapide (~1-2ms) — on l'exécute ici direct.
            _b._DH_SKIN_LIST_DIRTY   = False
            _b._DH_SKIN_BRIDGE_DIRTY = False

            try:
                import gclient.util.gun_skin_util as _sku
                _ca_now   = local.combat_avatar
                _gm_snap  = _b._DH_SKIN_GUN_MAIN
                _gs_snap  = _b._DH_SKIN_GUN_SUB
                _gmel_snap= _b._DH_SKIN_GUN_MELEE
                ...
                lst_main, lst_sub, lst_melee = [], [], []
                try:
                    wc_m = _ca_now.GetMainGunWeaponCase()
                    if wc_m: lst_main = _build(wc_m.gun_id)
                except: pass
                try:
                    wc_s = _ca_now.GetSubGunWeaponCase()
                    if wc_s: lst_sub = _build(wc_s.gun_id)
                except: pass
                try:
                    wc_mel = _ca_now.GetMeleeWeaponCase()
                    if wc_mel:
                        bp = wc_mel.body_equip_proto or {}
                        melee_wpn_id = bp.get('melee_weapon_id', 1)
                        for item in _sku.GetAllMeleeSkinItemList(melee_wpn_id):
                            try:
                                ...
                                lst_melee.append((guise_id, name, 'melee_guise'))
                            except: pass
                        lst_melee.sort(key=lambda x: x[1].lower())
                except: pass

                # Only write back if gun_ids haven't changed during build
                if (_b._DH_SKIN_GUN_MAIN  == _gm_snap and
                    _b._DH_SKIN_GUN_SUB   == _gs_snap and
                    _b._DH_SKIN_GUN_MELEE == _gmel_snap):
                    _b._DH_SKIN_LIST       = lst_main
                    _b._DH_SKIN_LIST_SUB   = lst_sub
                    _b._DH_SKIN_LIST_MELEE = lst_melee
                    _b._DH_SKIN_IDX_MAIN   = 0
                    _b._DH_SKIN_IDX_SUB    = 0
                    _b._DH_SKIN_IDX_MELEE  = 0
                    _b._DH_SKIN_BRIDGE_DIRTY = True
            except: pass

        # ── Build all-guns skin registry + write bridge file ─────────────────
        # ── Build ALL-GUNS skin bridge file ──────────────────────────────────
        # Scans ALL 44 known gun_ids using equip_data for gun names and
        # GetAllGunSkinItemList for skins. Written on load and on Refresh.
        # Format: GUN:<gun_id>:<gun_name>\nSKIN:<sid>:<kind>:<name>\n...END\n
        #
        # IMPORTANT: toutes les données C++ sont extraites ICI dans esp_tick
        # (thread Cocos). Le thread background ne reçoit que des données Python
        # pures → zéro accès aux objets C++ du jeu hors thread Cocos → zéro crash.
        if getattr(_b, '_DH_SKIN_BRIDGE_DIRTY', True) and local and local.combat_avatar \
                and getattr(_b, '_DH_ENFORCE_COUNTER', -999) >= 300:
            _b._DH_SKIN_BRIDGE_DIRTY = False
            try:
                ...
                # Extraire toutes les données C++ dans le thread Cocos
                _bridge_data = []  # [(gun_id, gun_name, [(sid, kind, name), ...]), ...]
                for _gun_id in ALL_GUN_IDS:
                    try:
                        _gname = _gun_name(_gun_id)
                        _skins = []
                        try:
                            for item in _sku2.GetAllGunSkinItemList(_gun_id):
                                sid = getattr(item, 'skin_id', None)
                                if not sid or sid <= 0: continue
                                ip   = getattr(item, 'item_proto', {}) or {}
                                name = _safe2(ip.get('name', ip.get('skin_name', str(sid))))
                                _skins.append((sid, 'skin', name))
                        except: pass
                        try:
                            for item in _sku2.GetGunGuiseItemAndMallGachaItemList(_gun_id):
                                gid2 = getattr(item, 'guise_id', None)
                                if not gid2 or gid2 <= 0: continue
                                ip   = getattr(item, 'item_proto', {}) or {}
                                name = _safe2(ip.get('name', ip.get('skin_name', str(gid2))))
                                _skins.append((gid2, 'guise', name))
                        except: pass
                        _skins.sort(key=lambda x: x[2].lower())
                        if _skins:
                            _bridge_data.append((_gun_id, _gname, _skins))
                    except: pass

                # Melee — aussi dans le thread Cocos
                _melee_data = None  # (weapon_id, mel_name, [(guise_id, name), ...]) ou None
                try:
                    _ca_bridge = local.combat_avatar
                    wc_mel = _ca_bridge.GetMeleeWeaponCase()
                    if wc_mel:
                        ...
                        if mskins:
                            _melee_data = (weapon_id, mel_name, mskins)
                except: pass

                # Thread background : I/O fichier uniquement, données Python pures
                import threading as _threading2
                _bd_copy  = _bridge_data
                _mel_copy = _melee_data

                def _write_bridge_bg(_bd, _mel):
                    try:
                        import os as _os2
                        lines = []
                        for gun_id, gun_name, skins in _bd:
                            lines.append(f'GUN:{gun_id}:{gun_name}')
                            for sid, kind, name in skins:
                                lines.append(f'SKIN:{sid}:{kind}:{name}')
                            lines.append('END')
                        if _mel:
                            weapon_id, mel_name, mskins = _mel
                        ...
                        _b._DH_SKIN_BRIDGE_DIRTY = True  # retry

                _threading2.Thread(target=_write_bridge_bg,
                                   args=(_bd_copy, _mel_copy),
                                   daemon=True).start()
            except: pass
        ...
        # ── Skin apply actions — only execute when skin changer is enabled ──────
        _skin_on = getattr(_b, '_DH_SKIN_ENABLED', False)

        # Prev/Next main
        if _skin_on and getattr(_b, '_DH_SKIN_NEXT_MAIN', False):
        ...
        if _skin_on and getattr(_b, '_DH_SKIN_PREV_MAIN', False):
        ...
        # Prev/Next sub
        if _skin_on and getattr(_b, '_DH_SKIN_NEXT_SUB', False):
        ...
        if _skin_on and getattr(_b, '_DH_SKIN_PREV_SUB', False):
        ...
        # Prev/Next melee
        if _skin_on and getattr(_b, '_DH_SKIN_NEXT_MELEE', False):
        ...
        if _skin_on and getattr(_b, '_DH_SKIN_PREV_MELEE', False):
        ...
        # Direct apply
        if _skin_on and getattr(_b, '_DH_SKIN_APPLY', False):
        ...
        if _skin_on and getattr(_b, '_DH_SKIN_APPLY_SUB', False):
        ...
        if cmd is not None and not _skin_on:
            _b._DH_SKIN_APPLY_CMD = None  # discard pending command when disabled
        elif cmd is not None and _skin_on and _space_valid():
        ...
        elif cmd is not None and _skin_on:
        ...
        if _skin_on and getattr(_b, '_DH_SKIN_CFG_SAVE', False):
            _b._DH_SKIN_CFG_SAVE = False
            _save_skin_config()

        if _skin_on and getattr(_b, '_DH_SKIN_CFG_CLEAR', False):
        ...
        def get_sbone(ent_ref, bone):
            ...
            try:
                ...
                if m is None:
                    ...
                    sp = ecam.GetScreenPointFromWorldPoint(m.GetBoneWorldPosition(bone))
                    if sp and sp.z != -1.0:
        ...
        # Snapshot du dict avant d'itérer — space.entities peut être modifié
        # depuis C++ (spawn/despawn) pendant l'itération → RuntimeError ou crash natif.
        try:
            _ent_snapshot = list(space.entities.values())
        except:
            _ent_snapshot = []

        idx = 0
        for ent in _ent_snapshot:
                ...
                if DO_DIST:
                    ...
                    if _dlbl_alive:
                        try:
                            ...
                            if _dm is not None:
                                origin   = ecam.GetOrigin()
                                head_pos = _dm.GetBoneWorldPosition('biped Head')
                                dist_m   = math.sqrt(
